...
//tmp portkonfig
 ...
 void motor_update_timer(void)
 {
	...
	if(motor_enable && direction)  
	{
		...
		tim_2_set_period((uint32_t)time_levels[current_level]);
	}
	else if(motor_enable && (!direction))
	{
		...
		current_pos--;	   
		//set_timer_1_ovf_value(time_levels[current_level]);	  // timer kompar�l�si ovf �rt�k --> frek frisst�s
		tim_2_set_period((uint32_t)time_levels[current_level]);
	} 
 ...
 void motor_1_init(void)
 {
	...
	//PORT_Init(&MOTOR_1_DIR_DIR,MOTOR_1_DIR_PIN, OUTPUT); 			// direction l�b

 }
 void motor_1_main(uint16_t dmx_pos_1)
 {
	uint16_t tim_state;									// timer jelenlegi �ll�sa 
	...
	//tim_state = get_timer_1_value();					// kiolvasni a timer �rt�k�t
	...
	{	  
		...
		//PORT_Write(&MOTOR_1_PULSE_PORT,MOTOR_1_PULSE_PIN,0);	// kimenet 0 ra �ll�t�sa
	}
 }
